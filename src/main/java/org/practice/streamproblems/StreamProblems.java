package org.practice.streamproblems;
...
//        customers.stream().sorted(Comparator.comparing(Customer::getName).thenComparing(Comparator.comparing(a->a.getId()).reversed())).toList();

        // Exercise 1 — Obtain a list of products belongs to category “Books” with price > 100
        products.stream().filter(p->p.getCategory().equals("Books")).filter(p->p.getPrice() > 100).forEach(System.out::println);
...
//        System.out.println("\n\nExercise 3 — Obtain a list of product with category = “Toys” and then apply 10% discount");
      products.stream().filter(p->p.getCategory().equals("Toys")).map(p->{
          p.setPrice(p.getPrice()*0.90);
                  return p;
      }
      ).forEach(System.out::println);

        //Exercise 4 — Obtain a list of products ordered by customer of tier 2 between 01-Feb-2021 and 01-Apr-2021
         List<Product> products1 =  orders.stream().filter(order -> order.getCustomer().getTier() == 2)
                 .filter(o->o.getOrderDate().isAfter(LocalDate.of(2021,02,01)) &&
                o.getOrderDate().isBefore(LocalDate.of(2021,04,01)))
                .flatMap(o-> o.products.stream()).toList();
        ...
        System.out.println("\n\nExercise 7 — Get a list of orders which were ordered on 15-Mar-2021, log the order records to the console and then return its product list");
        orders.stream().filter(o->o.getOrderDate().isEqual(LocalDate.of(2021,03,15)))
        ...
        // Note: The order of printing of orders and products is not sequential

        // Exercise 8 — Calculate total lump sum of all orders placed in Feb 2021
        System.out.println("\n\nExercise 8 — Calculate total lump sum of all orders placed in Feb 2021\n");
       double sum1 = orders.stream().filter(o->o.getOrderDate().getMonthValue() == 2 )
        ...
        // Note: sum() returns double

        // Exercise 9 — Calculate order average payment placed on 14-Mar-2021

        double avg = orders.stream().filter(o->o.getOrderDate().isEqual(LocalDate.of(2021,03,14)))
                .flatMap(order -> order.getProducts().stream())
                .mapToDouble(Product::getPrice)
                        .average().getAsDouble();
        ...
        System.out.println("\n\nExercise 12 — Produce a data map with order records grouped by customer");
       orders.stream().collect(Collectors.groupingBy(Function.identity() ,Collectors.groupingBy(Order::getCustomer)))
               .forEach((k,v) -> System.out.println(k.getCustomer().getName() + " : " + v));
        ...
        System.out.println("\n\nExercise 15 — Get the most expensive product by category");
       products.stream().collect(Collectors.groupingBy(Product::getCategory, Collectors.maxBy(Comparator.comparing(Product::getPrice))))
